...
const ALLOWED_CATEGORIES = ['Electronics', 'Clothing', 'Accessories', 'Books', 'Keys', 'Wallet', 'ID Card', 'Jewelry', 'Sports Equipment', 'Other'] as const;
...
type CandidateInput = { id?: unknown; title?: unknown; description?: unknown; category?: unknown; location?: unknown; type?: unknown };
...
const checkRateLimit = async (uid: string): Promise<void> => {
  const now = Date.now();
  const ref = firestore().collection('aiRateLimits').doc(uid);
  await firestore().runTransaction(async transaction => {
    ...
    if (now >= resetAt) { transaction.set(ref, { count: 1, resetAt: now + RATE_LIMIT_WINDOW_MS }); return; }
    if (count >= MAX_REQUESTS_PER_WINDOW) throw new HttpsError('resource-exhausted', 'AI request limit reached. Please try again later.');
...
const callGemini = async (contents: unknown, responseSchema: unknown): Promise<unknown> => {
  ...
  if (!apiKey) throw new HttpsError('failed-precondition', 'AI service is not configured.');
  const response = await fetch(`${GEMINI_URL}?key=${encodeURIComponent(apiKey)}`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ contents, generationConfig: { responseMimeType: 'application/json', responseSchema } })
  });
  if (!response.ok) { console.error('Gemini API request failed:', response.status); throw new HttpsError('internal', 'AI service request failed.'); }
  const payload = await response.json() as { candidates?: Array<{ content?: { parts?: Array<{ text?: unknown }> } }> };
  const text = payload.candidates?.[0]?.content?.parts?.find(part => typeof part.text === 'string')?.text;
  if (typeof text !== 'string' || text.length === 0) throw new HttpsError('internal', 'AI service returned an empty response.');
  try { return JSON.parse(text); } catch { throw new HttpsError('internal', 'AI service returned invalid structured data.'); }
};

const validateAnalysisResult = (value: unknown): AnalysisResult => {
  if (!value || typeof value !== 'object') throw new HttpsError('internal', 'AI returned an invalid analysis result.');
  const data = value as Record<string, unknown>;
  const { title, description, category, color, tags } = data;
  if (typeof title !== 'string' || title.length === 0 || title.length > 200 || typeof description !== 'string' || description.length > 2000 || typeof category !== 'string' || !ALLOWED_CATEGORIES.includes(category as typeof ALLOWED_CATEGORIES[number]) || typeof color !== 'string' || color.length > 100 || !Array.isArray(tags) || tags.length > 10 || tags.some(tag => typeof tag !== 'string' || tag.length > 100)) throw new HttpsError('internal', 'AI returned an invalid analysis result.');
  return { title, description, category, color, tags: tags as string[] };
};
...
const parseCandidateItems = (itemsJson: unknown): CandidateItem[] => {
  ...
  const json = itemsJson as string;
  let parsed: unknown;
  try { parsed = JSON.parse(json); } catch { rejectInvalid('The candidate item list must be valid JSON.'); }
  if (!Array.isArray(parsed) || parsed.length > MAX_ITEMS) rejectInvalid(`A maximum of ${MAX_ITEMS} candidate items is allowed.`);
  const items = parsed as unknown[];

  return items.map((rawItem: unknown, index: number): CandidateItem => {
...
export const analyzeItemImage = onCall({ secrets: [GEMINI_API_KEY], timeoutSeconds: 60 }, async request => {
  ...
  const image = imageBase64 as string;
  ...
  const cleanBase64 = image.replace(/^data:image\/(png|jpeg|jpg|webp);base64,/i, '');
  ...
  try {
    ...
    console.error('Image analysis failed:', error); throw new HttpsError('internal', 'Unable to analyze the image.');
  }
});
...
export const findSmartMatches = onCall(async request => {
  ...
  try {
    ...
    if (error instanceof HttpsError) throw error;
    console.error('Smart matching failed:', error); throw new HttpsError('internal', 'Unable to find smart matches.');
  }
});
