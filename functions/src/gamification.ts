 ...
 *
 * IMPORTANT:
 * The client never sends these values.
 *
 * The server decides them.
 *
 * These should match the values currently used by your app.
 */
 ...
 *
 * The client cannot manipulate the date used here.
 */
...
export const updateStreak = (
  ...
  if (
    !Array.isArray(streaks.weeklyActivity) ||
    streaks.weeklyActivity.length !== 7
  ) {
  ...
  /**
   * A user can only increase their streak once per day.
   */
  ...
  if (
    streaks.lastReportDate === yesterday ||
    !streaks.lastReportDate
  ) {
...
/**
 * ============================================================
 * MAIN SERVER-SIDE REWARD PROCESSOR
 * ============================================================
 */
...
export const processGamificationActivity = async (
  ...
  const [userSnapshot, itemSnapshot, awardSnapshot] =
    await transaction.getAll(userRef, itemRef, awardRef);
  ...
  /*
   * IMPORTANT:
   * Return/claim validation is intentionally kept conservative here.
   *
   * The current client still changes item status directly, so we should
   * NOT pretend that return/claim is fully secure yet.
   *
   * We will move those status transitions into trusted Cloud Functions
   * next, making the status change + reward atomic.
   */
  ...
  const achievementResult = checkAchievements(finalStats, now);

  finalStats = achievementResult.updatedStats;

  // ------------------------------------------------------------
  // Write authoritative stats
  // ------------------------------------------------------------
  ...
  // ------------------------------------------------------------
  // Record the award so it cannot happen twice
  // ------------------------------------------------------------
