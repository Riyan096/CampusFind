...
export const updateStreak = (stats: UserStats, now: Date) => {
  ...
  if (!Array.isArray(streaks.weeklyActivity) ||
...
export const checkAchievements = (stats: UserStats, now: Date) => {
  ...
  const newUserAchievements = newAchievements.map((achievement) => ({
    achievementId: achievement.id,
    unlockedAt,
    progress: 100,
  }));
  const achievementBonus = newAchievements.reduce(
    (total, achievement) => total + achievement.pointsBonus,
    0
  );
  return {
    newAchievements,
...
export const processGamificationActivity = async (
  ...
  const [userSnapshot, itemSnapshot, awardSnapshot] = await transaction.getAll(
    userRef,
    itemRef,
    awardRef
  );
  ...
  if (!itemSnapshot.exists) {
    throw new HttpsError("not-found", "Item was not found.");
  }
  ...
  const updatedStats: UserStats = {
    ...
    itemsReported: activityType === "report" ?
      stats.itemsReported + 1 : stats.itemsReported,
    itemsReturned: activityType === "return" ?
      stats.itemsReturned + 1 : stats.itemsReturned,
    itemsClaimed: activityType === "claim" ?
      stats.itemsClaimed + 1 : stats.itemsClaimed,
    lastActive: now.toISOString(),
  };
  const streakResult = activityType === "report" ?
    updateStreak(updatedStats, now) :
    {updatedStats, streakIncreased: false};
