...
import { deleteItemFromFirestore } from '../services/itemService';
import { doc, getDoc } from 'firebase/firestore';
import { db } from '../services/firebase';
import { useAuth } from '../context/AuthContext';
import { resolveItem } from '../services/itemApi';
import {
...
export const BrowseView: React.FC<BrowseViewProps> = React.memo(
  ({
    items,
    onItemsChange,
    onStatusChange,
    ...
    const [search, setSearch] = useState(() =>
      sanitizeSearchInput(searchQuery || '')
    );

    const [selectedItem, setSelectedItem] =
      useState<Item | null>(null);

    const [fetchedReporterName, setFetchedReporterName] =
      useState<string>('');

    const [filterType, setFilterType] =
      useState<ItemType | 'ALL'>('ALL');

    const [filterCat, setFilterCat] =
      useState<ItemCategory | 'ALL'>('ALL');

    const [advancedFilters, setAdvancedFilters] =
      useState<FilterState>(defaultFilters);

    const [activeTab, setActiveTab] =
      useState<'active' | 'resolved'>('active');

    // Helper to check if an item is resolved.
    ...
    // Fetch reporter name only when the item does not already
    // contain reporterName.
    useEffect(() => {
      if (
      ...
      const fetchReporterName = async () => {
        try {
          const userDoc = await getDoc(
            doc(db, 'users', selectedItem.reportedBy!)
          );

          if (userDoc.exists()) {
            const userData = userDoc.data();

            setFetchedReporterName(
              userData.displayName ||
                userData.email ||
                'Unknown User'
            );
          } else {
            setFetchedReporterName('Unknown User');
          }
        } catch (err) {
          console.error(
            'Error fetching reporter:',
            err
          );
    ...
    // Prefer the reporter name stored directly on the item.
    // Fall back to the fetched user name for legacy items.
    const reporterName =
      selectedItem?.reporterName ||
      fetchedReporterName ||
      '';
    ...
    // Memoize filtered items to avoid recalculation on
    // every render.
    const filteredItems = useMemo(() => {
      ...
      if (
        ...
        result = result.filter(item =>
          advancedFilters.locations.includes(
            item.location
          )
        );
      }
      ...
      if (
        ...
        result = result.filter(item =>
          advancedFilters.status.includes(
            item.status as
              | LostItemStatus
              | FoundItemStatus
          )
      ...
      if (
        ...
        result = result.filter(item =>
          advancedFilters.categories.includes(
            item.category
          )
        );
      }

      // Apply advanced type filter.
      if (
    ...
    const handleStatusChange = useCallback(
      async (
        ...
        try {
          // The server is authoritative for both the
          // status change and gamification.
          const result = await resolveItem(
            selectedItem.id,
            newStatus
          );

          // Always use the server-returned item.
          ...
          // Only send the notification for the first
    ...
    const handleDelete = useCallback(
      async (
        ...
        if (!canModifyItem(item)) {
          alert(
            'You can only delete items you reported.'
          );
          return;
        }
            ...
            filteredItems.map(item => (
              ...
              >
                ...
                >
                  ...
                  <div
                    className={`absolute top-3 right-3 text-xs font-bold px-2 py-1 rounded shadow-sm ${
                      item.type ===
                      ItemType.LOST
                ...
                >
                  ...
                  <div className="mt-auto pt-3 flex items-center gap-1 text-xs text-gray-500">
                    <span className="material-icons text-sm text-secondary">
                      location_on
                    </span>
          ...
          >
            ...
            >
              ...
              <div className="relative h-64 bg-gray-100 rounded-t-2xl overflow-hidden">
                ...
                <div
                  className={`absolute top-4 left-4 text-sm font-bold px-3 py-1.5 rounded-lg shadow-lg ${
                    selectedItem.type ===
                    ItemType.LOST
                      ? 'bg-red-500 text-white'
                      : 'bg-primary text-white'
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    ...
                    className={`px-6 py-3 border-2 border-gray-200 hover:border-primary text-gray-600 hover:text-primary font-bold rounded-lg transition-colors ${
                      user?.uid ===
                      selectedItem.reportedBy
                        ? 'flex-1'
                        : ''
